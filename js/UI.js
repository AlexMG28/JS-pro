class UI {
	...
	mainCard(tem, city, date, rh, min, max, icon, description) {
		...
		document.querySelector(".container .min").innerHTML = `<strong>Minimum Temperature: ${min}</strong>`;
		document.querySelector(".container .max").innerHTML = `<strong>Maximum Temperature: ${max}</strong>`;
	...
	forecast(celcius, f, min, max, day, icon) {
		...
		child.innerHTML = `
     	<img src="http://openweathermap.org/img/w/${icon}.png">
      	<h2 class="day">${day}</h2>
      	<h2>${celcius}ºC </h2>
      	<p><strong>min t: ${min}</strong></p>
      	<p><strong>max t: ${max}</strong></p>
    	`;
